...
export function particleWagmiWallet(param?: ConnectParam) {
    ...
    return createConnector<Provider, Properties>((config) => ({
        ...
        async getChainId() {
            ...
            return Number(chainId); // Use Number instead of normalizeChainId
        },
        async getProvider() {
        ...
        async isAuthorized() {
            try {
                const provider = await this.getProvider();
                return (provider as any)?.isConnected() ?? false;
            } catch {
                return false;
            }
        },
        ...
        onChainChanged(chain: string) {
            const chainId = Number(chain); // Use Number instead of normalizeChainId
